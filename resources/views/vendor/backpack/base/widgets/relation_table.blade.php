    ...
    <div class="row mb-0">
        <div class="col-sm-6">
            <div class="d-flex align-items-center mb-2">
                <h2 class="mr-2 mb-0">{{$widget['label']}}</h2>
                    ...
                    <div>
                        ...
                        >
                            <span class="ladda-label"><i
                                    class="la la-plus"></i> {{ trans('backpack::crud.add') }}</span>
                        </a>
                    </div>
                @endif
            </div>
            ...
            <tr role="row">
                @foreach($widget['columns'] as $column)
                    @php
                        ...
                        if(isset($column['type'])){
                            if($column["type"]=="video"){
                               $video =json_decode($item->{$column["name"]});
                               if($video!=null){
                    ...
                    @if(isset($column['type']) && $value!='')
                        @if($column["type"]=="video" && $video!=null)
                            <td>
                                <span>
                                    <a target="_blank" href="{{$video->url}}" title="{{$video->title}}"
                                       style="background: {{$bgColor}}; color: #fff; display: inline-block; width: 30px; height: 25px; text-align: center; border-top-left-radius: 3px; border-bottom-left-radius: 3px; transform: translateY(-1px);">
                                <i class="la la-{{$video->provider}}" style="transform: translateY(2px);"></i>
                            </a><img src="{{$video->image}}" alt="{{$video->title}}"
                                     style="height: 25px; border-top-right-radius: 3px; border-bottom-right-radius: 3px;"/>
                                </span>
                            </td>
                        @endif
